...
parser.add_argument('--original',default = False , type = bool , help ='evaluating original singan output')
...
def calculate_sifid_given_paths(path1, path2, batch_size, cuda, dims, suffix,original):
    ...
    if not original:
        ...
        Im_ind = []
        for fold in folders:
            pth = os.path.join(path2_ , fold)
            pth = pathlib.Path(pth)
            files2 = list(pth.glob('*.%s' %suffix))
            print('===========================================')
            for i in range(len(files2)):
                if files1[1].name[:-4] in files2[i].name:
                    m1, s1 = calculate_activation_statistics([files1[1]], model, batch_size, dims, cuda)
                    m2, s2 = calculate_activation_statistics([files2[i]], model, batch_size, dims, cuda)
                    m3, s3 = calculate_activation_statistics([files1[0]], model, batch_size, dims, cuda)
    
                    sifid_1 = calculate_frechet_distance(m1, s1, m2, s2)
                    sifid_2 = calculate_frechet_distance(m3, s3, m2, s2)
                    
                    our_metric = sifid_2 + sifid_1+abs(sifid_2 - sifid_1)
        
                    file_num1 = files1[1].name
                    file_num2 = files2[i].name +"  folder  : " +  fold
                    print('______________________________')
                    print("file" , file_num2)
                    #print(file_num1)
                    print("sifid sketch : ", sifid_1)
                    print("sifid real : ", sifid_2)
                    print("our metric", our_metric)
                    print('______________________________')
                    # Im_ind.append(int(file_num1[:-4]))
                # Im_ind.append(int(file_num2[:-4]))
    else:
        ...
        folders = os.listdir(path2_)
        # for fold in folders:
        #     for f in os.listdir(os.path.join(path2_ , fold)):
        #         i+=1
        #         os.rename(os.path.join(path2_ , fold)+"/"+f , os.path.join(path2_ , fold)+"/image"+str(i)+".jpg")
        # print(folders)
        # print(files1)
        ...
        for fold in folders:
            if files1[0].name[:-4] in fold:
                ...
                for i in range(len(files2)):
                        m1, s1 = calculate_activation_statistics([files1[0]], model, batch_size, dims, cuda)
                        m2, s2 = calculate_activation_statistics([files2[i]], model, batch_size, dims, cuda)
                        m3, s3 = calculate_activation_statistics([files1[1]], model, batch_size, dims, cuda)
...
if __name__ == '__main__':
    ...
    sifid_values = calculate_sifid_given_paths(path1,path2,1,args.gpu!='',64,suffix, original)
